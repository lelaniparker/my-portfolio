const project = [
  {
    name: "Ruby Assessment",
    title: "Shopping List App",
    content: [
      `Our first assessment was a Ruby Terminal App. I created it together with my classmate Jamie.`,
      `Our app is a shopping list where you can add items and quantities. You can select a category for each of your items. You have an option to view your current shopping list as well. When you're done, you can view your final shopping list and everything's sorted by category.`,
      `Looking back I am amazed that we managed to designed and build our app in only two days. It was an amazing experience and I learnt so much. My favourite part besides coding was doing pair-programming.`,
    ],
    techStack: "Ruby, CLI",
    githubLink: "https://github.com/Lelani82/Terminal-App-Assessment",
  },
  {
    name: "Portfolio Assessment",
    title: "Portfolio Website",
    content: [
      `Our second assessment was to create a fully-responsive portfolio website. Here we had to create a platform where we could share our relevant skills and a bit about ourselves to potential employers/ clients.`,
      `I included an 'About' page where you can learn a little more about me, as well as a 'Projects' page to showcase my older projects`,
    ],
    techStack: "HTML, CSS, SASS",
    websiteLink: "http://lelaniparker.design",
    githubLink: "https://github.com/Lelani82/Lelani82.github.io",
  },
  {
    name: "Ruby on Rails Assessment",
    title: "Shooz Online Shoe Shop",
    content: [
      `Our third assessment was a Two-Sided Marketplace App using Ruby on Rails. This was created together with Rachel, another one of my classmates.`,
      `Our app allows a user to purchase or sell high-end used shoes. Shoes are categorised by Boots, Flats, Heels and Trainers.`,
      `I really enjoyed the project management and wireframe creation. My favourite part was using Agile Methodology.`,
    ],
    techStack: "Ruby, Rails, PostgreSQL, RSpec, HTML, CSS",
    websiteLink: "https://sheltered-ridge-97373.herokuapp.com/",
    githubLink: "https://github.com/raeginez/railsproject",
  },
  {
    name: "JavaScript Hackathon",
    title: "Tony Time Website",
    content: [
      `After learning Vanilla JavaScript we had a class hackathon.`,
      `Our team consisted of three people including myself, and we created a cocktail selection website using JavaScript API's`,
    ],
    techStack: "HTML, CSS, JavaScript",
    websiteLink: "http://lelaniparker.design/tony_time",
    githubLink: "https://github.com/Lelani82/tony_time",
  }
];

export default project;
